(ns thales-combat.main
  "Kotodama WASM entrypoint for the Thales_combat clean-room actor (L4) — Clojure port.
  ...
  facts (`thales_combat.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "thales_combat")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "thalesco_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "thalesco_par"
    :fields [:caseId :role :name]             :required [:role :name]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents"  :id-prefix "thalesco_doc"
    :fields [:caseId :type :contentRef]      :required [:type]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents"  :id-prefix "thalesco_inc"
    :fields [:type :jurisdiction :occurredAt] :required [:type :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "thalesco_wat"
    :fields [:name :program :entries]        :required [:name :program]
    :coerce {:entries :int}          :refs {}}
   {:entity "Sanction"  :plural "sanctions"  :id-prefix "thalesco_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"thales_combat.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "thales_combat-compat" :tier tier :entities entities} 200])
...
(defn start! [] :thales_combat-compat/ready)
